...
public class DiffieHellman {
    ...
    //키 생성과 모듈 연산을 위해 prime number인 p와 g 선택
    ...
    public DiffieHellman() {
        this.p = new BigInteger(hexStrOfP.replace("\n", "").replace(" ", ""), 16); //줄바꿈과 스페이스 제거
        this.g = 2; //미리 정함 --> 시간 절약
    ...
    private BigInteger getRandomPrime() {
        ...
        UniformIntegerDistribution uniformIntegerDistribution = new UniformIntegerDistribution(
                randomGenerator, 0, 255);
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < 256; i++) {
            builder.append(Integer.toHexString(uniformIntegerDistribution.sample()));
        }
        return new BigInteger(builder.toString(), 16).nextProbablePrime();
    }
